...
<head>
    ...
    <style>
        * {
        ...
        .week-management, .day-off-selection, .holiday-management {
            margin-bottom: 1.5rem;
        }

        .week-management h4, .day-off-selection h4, .holiday-management h4 {
            margin-bottom: 1rem;
            color: #2c3e50;
        }

        .week-inputs, .holiday-inputs {
            display: grid;
        ...
        .week-item:last-child, .holiday-item:last-child {
            border-bottom: none;
        }
...
<body>
    <div class="App">
        <header class="app-header">
            <div class="app-title">
                ...
                <div class="app-subtitle">Made By MuNnA 🇧🇩</div>
            </div>
        ...
        <div class="settings-modal" id="settingsModal">
            <div class="settings-content">
                ...
                <div class="week-management">
                    ...
                    <!-- Hidden existing weeks section -->
                    <div style="display: none;">
                        <h4>Existing Weeks</h4>
                        <div class="week-list" id="weekList">
                            <!-- Weeks will be listed here but hidden from view -->
                        </div>
                    </div>
                </div>
                ...
                <div class="holiday-management">
                    ...
                    <!-- Hidden existing holidays section -->
                    <div style="display: none;">
                        <h4>Existing Holidays</h4>
                        <div class="holiday-list" id="holidayList">
                            <!-- Holidays will be listed here but hidden from view -->
                        </div>
                    </div>
    ...
    <script>
        ...
        function loadUserSettings() {
            const savedDaysOff = localStorage.getItem('warehouseDaysOff');
            if (savedDaysOff) {
        ...
        function deleteWeek(id) {
            if (confirm('Are you sure you want to delete this week? This will not delete performance records.')) {
        ...
        // Render week list (kept for functionality but hidden from view)
        ...
        function deleteHoliday(id) {
            if (confirm('Are you sure you want to delete this holiday?')) {
                ...
                if (holidayToDelete && holidayToDelete.isRange) {
        ...
        // Render holiday list (kept for functionality but hidden from view)
        ...
        function saveSettings() {
            ...
            localStorage.setItem('warehouseDaysOff', JSON.stringify(daysOff));
            closeSettings();
        ...
        function getSelectedMonthData(selectedMonth) {
        ...
        function getAvailableMonths() {
            const months = [...new Set(performanceData.map(record => 
                ...
                .forEach(({week, data}) => {
                    ...
                    html += `
                        <tr>
                            ...
                            <td><span class="bonus-amount">£${performance.bonus.toFixed(2)}</span></td>
                            <td>${workDays}/5</td>
        ...
        function renderMonthlyPerformance() {
            if (performanceData.length === 0) {
                return '<div class="table-container"><h3>Monthly Performance Summary</h3><p>No records yet. Add some performance data to get started.</p></div>';
            }
            
            const months = {};
            performanceData.forEach(record => {
                const monthNo = new Date(record.date).getMonth() + 1;
                if (!months[monthNo]) {
                    months[monthNo] = [];
                }
                months[monthNo].push(record);
            });
            ...
            Object.entries(months)
                .sort(([a], [b]) => a - b)
                .forEach(([monthNo, monthData]) => {
                    const performance = calculateMonthlyPerformance(monthData);
                    const holidaysInMonth = monthData.filter(day => isHoliday(day.date))
                        .map(day => getHolidayName(day.date));
                    ...
                    html += `
                        <tr>
                            ...
                            <td><span class="bonus-amount">£${performance.totalBonus.toFixed(2)}</span></td>
        ...
        function downloadMonthlyRecords() {
            ...
            Object.entries(months)
                .sort(([a], [b]) => a - b)
                .forEach(([monthNo, monthData]) => {
                    monthData.sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(record => {
                        let status = 'Working Day';
                        if (daysOff.includes(record.dayName)) {
                            status = 'Day Off';
                        } else if (isHoliday(record.date)) {
                        ...
                        csvContent += `"${record.userName || 'Unknown User'}","Month ${monthNo}","${record.date}","${record.dayName}",${record.picking},${record.loading},"${record.oneShift ? 'Yes' : 'No'}",${record.dailyPerformance.toFixed(2)}%,"${status}"\n`;
                    });
                });
        ...
        function renderMonthlyChart(months) {
            ...
            const monthLabels = Object.keys(months).sort((a, b) => a - b).map(m => `Month ${m}`);
            const pickingData = Object.keys(months).sort((a, b) => a - b).map(month => {
                const performance = calculateMonthlyPerformance(months[month]);
                return performance.totalPicking;
            });
            const loadingData = Object.keys(months).sort((a, b) => a - b).map(month => {
                const performance = calculateMonthlyPerformance(months[month]);
                return performance.totalLoading;
            });
            const averageData = Object.keys(months).sort((a, b) => a - b).map(month => {
                const performance = calculateMonthlyPerformance(months[month]);
        ...
        function updateWeekContent(selectedWeekId) {
            ...
            html += `
                ...
                <div class="summary">
                    ...
                    <p>Performance Bonus: <span class="bonus-amount">£${performance.bonus.toFixed(2)}</span></p>
                </div>`;
        ...
        function renderSpecificMonth() {
            ...
            availableMonths.forEach(month => {
                html += `<option value="${month}">Month ${month}</option>`;
            });
        ...
        function updateMonthContent(selectedMonth) {
            ...
            let html = `
                <h3>Month ${selectedMonth} Performance</h3>
                <table class="performance-table">
                    <thead>
                        <tr>
                            <th>Week</th>
            ...
            performance.weeks.forEach((week, index) => {
                html += `
                    <tr>
                        <td>Week ${index + 1}</td>
                        <td>${week.picking}</td>
                        <td>${week.loading}</td>
                        <td><span class="percentage">${week.average.toFixed(2)}%</span></td>
                        <td><span class="bonus-amount">£${week.bonus.toFixed(2)}</span></td>
                    </tr>`;
            });
            ...
            html += `
                ...
                <div class="summary">
                    <h4>Month ${selectedMonth} Summary</h4>
                    ...
                    <p>Total Bonus: <span class="bonus-amount">£${performance.totalBonus.toFixed(2)}</span></p>
                </div>`;
